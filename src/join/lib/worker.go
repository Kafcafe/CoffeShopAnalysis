...
func (j *JoinGenericWorker) joinWithPayload(message amqp.Delivery) error {
	...
	j.watchMesh.TryCrash("before processing message")
	...
	j.watchMesh.TryCrash("after processing message - before stats update")
	clientStats.Add(msg.DataType, message.MessageId, true, false, "users", partialUpdate)
	j.watchMesh.TryCrash("after processing message - between stats update and save worker state")
...
func (j *JoinGenericWorker) joinWithSideTable(message amqp.Delivery) error {
	...
	j.watchMesh.TryCrash("after processing message - before ack")
	answerMessage(ACK, message)
	return nil
}
...
func (j *JoinGenericWorker) Run() error {
	...
	j.watchMesh.TryCrash("before recover")
	if err := j.Recover(); err != nil {
		return fmt.Errorf("failed to recover state: %v", err)
	}
	j.watchMesh.TryCrash("after recover")
