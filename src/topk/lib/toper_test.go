...
func ValueTransaction(line string) float64 {
	fields := strings.Split(line, ",")
	var value float64
	_, err := fmt.Sscanf(fields[7], "%f", &value)
	if err != nil {
		return 0.0
	}
	return value
...
func TestGenerateTopKFromTransactions(t *testing.T) {
	topper := heap.NewToper(5, CmpTransactions)
	...
	require.EqualValues(t, expected, topK)
}
