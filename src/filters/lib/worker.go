...
func (f *FilterGenericWorker) filterMessage(message amqp.Delivery) error {
	...
	f.watchMesh.TryCrash("before processing message")
	...
	if len(filteredBatch) == 0 {
		f.watchMesh.TryCrash("after processing message - before stats update")
		f.getClientStats(msg.ClientId).Add(msg.DataType, message.MessageId, true, false)
		f.watchMesh.TryCrash("after processing message - before save worker state")
		f.saveWorkerState(msg.ClientId)
		f.watchMesh.TryCrash("after processing message - before ack")
	...
	f.watchMesh.TryCrash("after processing message - between stats update and save worker state")
	f.saveWorkerState(msg.ClientId)
	f.watchMesh.TryCrash("after processing message - before ack")
...
func (f *FilterGenericWorker) Run() error {
	...
	f.watchMesh.TryCrash("before createExchangeHandlers")
	err := f.createExchangeHandlers()
	if err != nil {
		return fmt.Errorf("failed to create exchange handlers: %v", err)
	}

	f.watchMesh.TryCrash("before recover")
	f.recover()
	f.watchMesh.TryCrash("after recover")
